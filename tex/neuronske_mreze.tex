...
\begin{equation}
	...
	\end{equation}
\textcolor{red}{Robins i Monro su u svom radu \cite{robins_monro} pokazali da pri ovakvom odabiru veličine koraka iterativni proces \ref{eq:gradijentni_spust} konvergira. Na primer, $\alpha_k = \frac{1}{k}$ zadovoljava Robins Monroove uslove.
...
Algoritam RMSProp (eng.~{\em root mean square propagation}) predložio je Džefri Hinton (eng.~{\em Geoffrey Hinton}) na jednom od svojih predavanja na sajtu Kursera \cite{rmsprop}. Ovo je algoritam optimizacije korišćen prilikom razvijanja DQN algoritma. Glavna ideja je čuvanje dosadašnjeg otežanog proseka kvadrata gradijenta funkcije koji će biti obeležen sa $g_k$. \textcolor{red}{Simbol $\odot$ obeležava pokoordinatno množenje (eng.~{\em element-wise multiplication}) dva vektora, odnosno Adamarov (fr.~{Hadamard}) proizvod: 
\begin{equation}
...
\end{figure}
\par 
\textcolor{red}{Rezultat primene konvolucije za sve validne pozicije $(x, y)$ manjih je dimenzija od početnog signala, $I$. Ukoliko je signal $I$ dimenzija $k \times l$ a filter $K$ dimenzija $p \times q$, tada je $I \ast K$ dimenzija najviše $k-p+1 \times l-q+1$. Moguće je i vršiti horizontalno i vertikalno preskakanje pozicija prilikom računanja konvolucije, što dodatno smanjuje dimenziju rezultata. Razlika između uzastopnih pozicija za koje se računa konvolucija sa filterom naziva se pomerajem (eng.~{\em stride}).
